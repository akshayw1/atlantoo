...
class CorrelationService {
  ...
  async correlateByTimeWindow(params) {
    const { 
      ...
      startTime, 
      endTime,
      threshold = 0.05 // Default error threshold (5%)
    } = params;
    ...
    if (allAnomalies.length > 0) {
      ...
      if (incident) {
        await this._triggerAISolutionAnalysis(incident._id, correlationId);
      }
  ...
  try {
    ...
    const response = await axios.post(
      `${this.aiAnalyzerUrl}/api/detect-anomalies`,
      requestData,
      { timeout: 10000 }
    );
  ...
  async _createIncident(serviceName, timeRange, anomalies, severity, impactedServices) {
    try {
      ...
      // Create incident title based on anomaly types
      const primaryAnomalyType = this._getPrimaryAnomalyType(anomalies);
      const title = `${primaryAnomalyType} detected in ${serviceName}`;
  ...
  async _triggerAISolutionAnalysis(incidentId, correlationId) {
    try {
      const response = await axios.post(
        `${this.aiAnalyzerUrl}/api/analyze/incident`,
        { incidentId, correlationId },
        { timeout: 15000 }
      );
      
      logger.info(`Triggered AI solution analysis for incident ${incidentId}`);
